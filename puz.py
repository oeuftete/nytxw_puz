...
ENCODING = "ISO-8859-1"
ENCODING_UTF8 = "UTF-8"
# raises an exception for bad chars; change to 'replace' for laxer handling
...
class Puzzle:
    ...
    def load(self, data):
        ...
        self.encoding = ENCODING if self.version_tuple()[
            0] < 2 else ENCODING_UTF8
    ...
    def clue_numbering(self):
        numbering = DefaultClueNumbering(
            self.fill, self.clues, self.width, self.height)
    ...
    def magic_cksum(self):
        ...
        for (i, cksum) in enumerate(reversed(cksums)):
            cksum_magic <<= 8
            cksum_magic |= ord(
                MASKSTRING[len(cksums) - i - 1]) ^ (cksum & 0x00FF)
            cksum_magic |= (
...
class Rebus:
    ...
    def save(self):
        if self.has_rebus():
            ...
            rebus_solutions = self.puzzle.encode(
                dict_to_string(self.solutions))
